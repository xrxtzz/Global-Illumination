	...
	//for (int i=0;i<pathnum;i++) {
		...
		if (cosphi > 0) {
			...
			if(!shadowhit.getPrimitive()) {
				...
				res += light_color*(2000 * radianceTransfer * brdf);
			}
		}
	//}
	result = res;
                               ...
                               const Ray& ray, const HitRecord& hit, const Color&, int) const
{
	...
	directIllumination(direct, ray, hit, context);
	indirectIllumination(indirect, ray, hit, context);
	result = (direct + indirect) * color;
}
	/*
  const Scene* scene = context.getScene();
  const vector<Light*>& lights = scene->getLights();
