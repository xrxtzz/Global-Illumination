...
Sphere::Sphere(Material* material, const Point& center1, double radius)
  : Primitive(material),  center(center1.x(), center1.y(), center1.z()), radius(radius)
{
  inv_radius = 1./radius;
}
...
void Sphere::move(const Vector& motion){
	center = Point(center.x()+motion.x(), center.y()+motion.y(), center.z()+motion.z());
}
