...
AreaLight *Parser::parseAreaLight()
{
  Point position( 0.0, 0.0, 10.0 );
  Color color( 1.0, 1.0, 1.0 );
  double size = 0;
  ...
  return new AreaLight( position, color, size );
}

AreaLight *Parser::parseLight()
{
    //if ( peek( "point" ) )
...
Object *Parser::parseTriangleObject()
{
	...
	return new Triangle(material, normal, point1, point2, point3);
}
  ...
  string &filename )
{
  ...
  for ( ; ; ) {
    if ( peek( "filename" ) )
      ...
      scene->setSample( parseInteger());
	else if ( peek( "photon") )
		scene->setPhoton( parseInteger());
    else if ( peek( "F" ) )
     scene->setF( parseReal());
    else if ( peek( "D" ) )
      scene->setD( parseReal());
    else if ( peek( "n" ) )
      scene->setn( parseInteger());
    else if ( peek( "motion"))
	  scene->setMotion( parseVector());
    else if ( peek( "EnableAntiAliasing"))
		scene->controlAntiAliasing( parseInteger());
	else if ( peek( "EnableDepthofField"))
		scene->controlDepthofField( parseInteger());
	else if ( peek( "EnableMotionBlur"))
		scene->controlMotionBlur( parseInteger());
